...
def CanWinGame(state: CollectionState, locentr: LocEntrDef, world: MultiWorld, player: int, options, argument: Tuple = None):
    ...
    # Python 3.8 IntFlag values are not iterable, so test each defined flag explicitly.
    return all(passes_rule(rule, *args) for flag, rule in win_conditions.items() if win_condition & flag)
